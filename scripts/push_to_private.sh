...
# Stash any uncommitted changes (to restore later)
git stash push -u -m "Temporary stash before pushing to private repo"
...
# Revert to the original state before forcefully adding files
# Pop the last stash (restore uncommitted changes and untracked files)
